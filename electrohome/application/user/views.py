...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, 
                f'Registro exitoso! Hemos enviado un correo de verificacin a {user.email}. '
...
def verify_email(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(
            request, 
            'Correo verificado exitosamente! Tu cuenta ha sido activada. Ahora puedes iniciar sesin.'
        )
        return redirect('user:verification_success')
    else:
        messages.error(
            request, 
            'El enlace de verificacin es inv!lido o ha expirado. '
            'Por favor solicita un nuevo correo de verificacin.'
        )
...
def resend_verification(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if user.is_active:
                messages.info(request, 'Esta cuenta ya est! verificada. Puedes iniciar sesin.')
                return redirect('user:login')
            # send_verification_email(request, user)
            messages.success(
                request, 
                f'Correo de verificacin reenviado a {email}. Revisa tu bandeja de entrada.'
            )
            return redirect('user:verification_sent')
        except Usuario.DoesNotExist:
            messages.error(request, 'No existe una cuenta con este correo electrnico.')
    
    return render(request, 'user/resend_verification.html')


# ============================================
#  LOGIN - REDIRIGE SEGN TIPO DE USUARIO
# ============================================
...
def login_view(request):
    if request.user.is_authenticated:
        # Si ya est! autenticado redirigir segn su tipo
        ...
        if form.is_valid():
            ...
            try:
                ...
                if not user_check.is_active:
                    messages.error(
                        request, 
                        'Tu cuenta no ha sido verificada. Por favor revisa tu correo electrnico. '
                        '<a href="/user/resend-verification/" class="text-blue-600 font-bold underline">Reenviar correo de verificacin</a>',
                        extra_tags='html'
                    )
            ...
            if user:
                ...
                #  REDIRECCIN SEGN TIPO DE USUARIO
                if user.is_superuser:
                    # Admin  panel de administracin Django
                    return redirect('/admin/')
                ...
                messages.error(request, 'Correo o contrasea incorrectos.')
        else:
...
def logout_view(request):
    ...
    logout(request)
    messages.success(request, f'Hasta pronto, {user_name}! Has cerrado sesin correctamente.')
    return redirect('product:home')
    ...
    if request.method == 'POST':
        ...
        if telefono:
            if not re.match(r'^\d{10}$', telefono):
                errores.append('El telfono debe tener exactamente 10 nmeros')
...
def access_denied(request):
    messages.warning(request, 'Debes iniciar sesin para acceder a esta p!gina.')
    return redirect('user:login')
