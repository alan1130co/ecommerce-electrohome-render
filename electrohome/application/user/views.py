...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # ★ GUARDAR SESSION KEY ANTES DE login()
            # Forzar que la sesión exista y esté persistida en BD antes de que
            # Django la regenere al autenticar, para no perder el carrito anónimo.
            if not request.session.session_key:
                request.session.create()
            request.session.save()
            old_session_key = request.session.session_key

            # ★ DEBUG TEMPORAL — ver en consola del servidor qué pasa
            logger.warning(f"[CART MERGE] old_session_key capturado: {old_session_key}")
            from application.product.models import Cart as CartDebug
            cart_debug = CartDebug.objects.filter(session_key=old_session_key, user__isnull=True).first()
            logger.warning(f"[CART MERGE] carrito anonimo encontrado: {cart_debug}")
            if cart_debug:
                logger.warning(f"[CART MERGE] items en carrito anonimo: {cart_debug.items.count()}")

            user = authenticate(request, username=email, password=password)
            if user:
                login(request, user, backend='django.contrib.auth.backends.ModelBackend')
                messages.success(request, f'Ha iniciado sesión exitosamente como {user.first_name or user.email}!')
                ...
                if user.is_superuser:
                    ...
                    if old_session_key:
                        try:
                            ...
                            # Buscar solo carritos anónimos (sin usuario asignado)
                            session_cart = Cart.objects.filter(
                                session_key=old_session_key,
                                user__isnull=True
                            ).first()

                            logger.warning(f"[CART MERGE] session_cart post-login: {session_cart}")
                            ...
                            if session_cart and session_cart.items.exists():
                                ...
                                for item in session_cart.items.all():
                                    ...
                                    if existing:
                                        ...
                                        existing.save()
                                        logger.warning(f"[CART MERGE] sumado: {item.product.nombre}")
                                    else:
                                        item.pk = None
                                        item.cart = user_cart
                                        item.save()
                                        logger.warning(f"[CART MERGE] movido: {item.product.nombre}")

                                session_cart.delete()
                                logger.warning(f"[CART MERGE] merge exitoso: session {old_session_key} -> usuario {user.email}")
                            else:
                                logger.warning(f"[CART MERGE] no habia carrito anonimo con items para migrar")

                        except Exception as e:
                            logger.error(f"[CART MERGE] ERROR al migrar carrito: {type(e).__name__}: {e}")

                    next_url = request.GET.get('next') or 'product:home'
